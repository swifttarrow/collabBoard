-- Board history: store applied ops with full payload for cross-user history and restore.
-- Enables viewing and restoring to any point in time across all users.
...
create index if not exists idx_board_history_board_revision on board_history(board_id, server_revision);
create index if not exists idx_board_history_board_created on board_history(board_id, created_at);
alter table board_history enable row level security;

create policy "board_history_select" on board_history
  for select using (
    exists (
      select 1 from boards b
      left join board_members bm on bm.board_id = b.id
      where b.id = board_history.board_id
        and (b.owner_id = auth.uid() or bm.user_id = auth.uid())
    )
  );

-- Update apply_board_operation to record history when an op is applied
...
begin
  ...
  -- Record in history for cross-user restore
  insert into board_history (board_id, op_id, server_revision, op_type, payload, user_id, created_at)
  values (p_board_id, p_op_id, v_revision, p_op_type, p_payload, auth.uid(), now());
