-- Restore board_history insert in apply_board_operation.
-- Migration 0013 replaced the function for parent_id resilience but accidentally dropped
-- the history insert. Version history panel depends on board_history for cross-user restore.
-- Without this, no ops are recorded and "No history yet" appears for all users.
...
begin
  ...
  case p_op_type
    when 'create' then
      ...
      if v_parent_id is not null then
        select exists(
          select 1 from board_objects
          where id = v_parent_id and board_id = p_board_id
  ...
  -- Record in history for version history panel (cross-user restore)
