...
function suggestPlacement(objects: Record<string, BoardObjectWithMeta>): {
  ...
  if (entries.length === 0)
    return { startX: DEFAULT_START, startY: DEFAULT_START };
...
export async function createStickies(
  ...
  const stickyCount = Object.values(ctx.objects).filter(
    (o) => (o as { type?: string }).type === "sticky",
  ).length;
  console.log("[createStickies] called", {
    rawStickiesLength: Array.isArray(params.stickies)
      ? params.stickies.length
      : 0,
  ...
  const stickies = Array.isArray(params.stickies)
    ? params.stickies.filter(
        (s) => s != null && typeof (s as { text?: unknown }).text === "string",
      )
    : [];
  if (stickies.length === 0) {
    console.log(
      "[createStickies] early return: no valid stickies after filter",
    );
  ...
  try {
    ...
    const bottomY =
      startY +
      (rowOffsets[lastRow] ?? 0) +
      (rowHeights[lastRow] ?? 0) +
      GRID_GAP;
